...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let service =
        node.create_service::<HelloWorld::Service>("/hello_world", r2r::QosProfile::default())?;
    ...
    loop {
        ...
        if let Ok(mut clock) = node.get_ros_clock().lock() {
            println!("clock: {:?}", clock.get_now().unwrap_or_default());
        }
    }
}
