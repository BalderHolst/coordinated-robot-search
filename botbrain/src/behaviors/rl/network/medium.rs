//! Implementation of [`MediumNetwork`].
...
#[derive(Debug, Module)]
pub struct MediumNetwork<B: Backend> {
    linear1: Linear<B>,
    linear2: Linear<B>,
    linear3: Linear<B>,
    linear4: Linear<B>,
...
impl<B: Backend> MediumNetwork<B> {
    const HIDDEN_SIZE_1: usize = 30;
    const HIDDEN_SIZE_2: usize = 40;
    const HIDDEN_SIZE_3: usize = 30;
    const HIDDEN_SIZE_4: usize = 20;
}

impl<B: AutodiffBackend, S: State, A: Action> AutodiffNetwork<B, S, A> for MediumNetwork<B> {}

impl<B: Backend, S: State, A: Action> Network<B, S, A> for MediumNetwork<B> {
    fn init(device: &B::Device) -> Self {
        Self {
            ...
            linear5: LinearConfig::new(Self::HIDDEN_SIZE_4, A::SIZE).init(device),
        }
    }
    ...
    fn soft_update(this: Self, that: &Self, tau: f64) -> Self {
        Self {
            ...
            linear5: soft_update_linear(this.linear5, &that.linear5, tau),
        }
    }
}
