...
Localization is handled by the AMCL (Adaptive Monte Carlo Localization) algorithm \cite{amcl} \cite{amcl-docs} from the ROS 2 \texttt{nav2} stack \cite{nav2} \cite{nav2-docs}. AMCL uses LiDAR and odometry data to estimate the robot’s pose through a particle filter, yielding a position and orientation with an associated uncertainty.\\
