...
In some situations, the robot may not be able explore new area because of the limited area used for calculating the gradient of the serach grid.
In the case the search gradient is too small we can do a more expensive operation to guide the robot to a more rewarding area.
In other situations, where computational resources are not limited, the robot can solely rely on the pathing algoritms for control.

The general setup of the planner is explained in the follow subsections:
\subsubsection{Costmap}\label{sec:costmap}
First a way of representing the area already search and obstacles in the world is needed. 
The costmap has a resolution of $0.2m$ and is represented as a grid of cells.
\begin{itemize}
...
From the costmap, a way to find a suitable goal is required. This project uses frontier exploration to find all the frontiers. A frontier is a cell that seperates seached and unknown areas. The frontiers are found via a breadth first search fron the postion of the robot. This is done to avoid searching the whole costmap which leads to faster goal generation. When the frontiers is found, they are grouped into regions. 
...
\end{enumerate}
The distance from the frontier to the nearest obstacles also has to be furter than a parameter specifying the clearence the robot should have to obstacles.
% TODO: Evaluate the used parameters.
\subsubsection{A-star path planning}\label{sec:a_star}
Once a goal has been generated, a path to that goal needs to be generated. First a stright line to the goal is made. If this path can be validated, meaning the clearance to obstacles is uphold, the robot will simply follow this path.
I situations where this clearance can't be validated, the a-star algorithm is used.
The a-star algorithm uses the distance
The a-star algorithm is implemented as a max-heap, where the 
% TODO: Explain A-star and straight line path generation
\subsubsection{Path following}\label{sec:path_following}
% TODO: How do we follow and validate the path?
...
% TODO: Pathing outside the proxtimity grid.
