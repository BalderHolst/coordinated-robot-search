...
def generate_launch_description():
    ...
    headless = False
    world = os.path.join(
        get_package_share_directory("testlaunch"),
        "configs",
        "worlds",
        "tb3_sandbox.sdf",
    )
    map = os.path.join(
        get_package_share_directory("testlaunch"), "configs", "maps", "tb3_sandbox.pgm"
    )
    rviz_config_file = os.path.join(
        get_package_share_directory("testlaunch"), "configs", "rviz", "rviz.sdf"
    )

    robot_sdf = os.path.join(
        get_package_share_directory(""), "configs", "rviz", "rviz.sdf"
    )
